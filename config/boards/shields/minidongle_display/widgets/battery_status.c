...
#if IS_ENABLED(CONFIG_ZMK_MINIDONGLE_DISPLAY_DONGLE_BATTERY)
...
static void set_battery_symbol(lv_obj_t *widget, struct battery_state state) {
    if (state.source >= 1 + SOURCE_OFFSET) {
        return;
    }
...
#endif /* IS_ENABLED(CONFIG_ZMK_MINIDONGLE_DISPLAY_DONGLE_BATTERY) */
...
int zmk_widget_dongle_battery_status_init(struct zmk_widget_dongle_battery_status *widget, lv_obj_t *parent) {
    ...
    for (int i = 0; i < 1 + SOURCE_OFFSET; i++) {
        ...
        lv_obj_align(battery_label, LV_ALIGN_BOTTOM_LEFT, 0, -i * 20);
